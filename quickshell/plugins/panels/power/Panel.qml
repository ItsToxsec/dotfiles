...
Panel {
  ...
  readonly property bool discharging: root.batteryState === "Discharging"
  readonly property real batteryFraction: Math.max(0, Math.min(1, Number(root.batteryInfo.fraction || 0)))
  readonly property bool fullyCharged: root.batteryInfo.status === "Full" || root.batteryFraction >= 0.999
  readonly property bool charging: root.batteryInfo.status === "Charging"
  readonly property bool chargeThresholdActive: false
  readonly property bool batteryFull: fullyCharged
  readonly property bool batteryFlowIdle: batteryFull
  readonly property string batteryState: String(root.batteryInfo.status || "Unknown")

  function batteryIcon() {
    var p = root.batteryFraction
    if (root.charging) return "󰂅"
    if (p >= 0.9) return "󰁹"
    if (p >= 0.75) return "󰂀"
    if (p >= 0.5) return "󰁽"
    if (p >= 0.25) return "󰁻"
  ...
  function updateProfiles(raw) {
    ...
    if (parsed.profiles.length === 0) {
      profileError = "Power profiles unavailable"
      return
    }
    profileError = ""
  ...
  Process {
    id: batteryProc
    command: ["sh", "-c", "BAT=/sys/class/power_supply/BAT1; cap=$(cat \"$BAT/capacity\" 2>/dev/null || echo -1); status=$(cat \"$BAT/status\" 2>/dev/null || echo Unknown); if [ -r \"$BAT/energy_full\" ]; then ef=$(cat \"$BAT/energy_full\"); size=$(awk -v x=\"$ef\" 'BEGIN{printf \"%.2f Wh\", x/1000000}'); else size=\"\"; fi; awk -v c=\"$cap\" -v st=\"$status\" -v sz=\"$size\" 'BEGIN{printf \"percentage\\t%s%%\\nfraction\\t%.4f\\nstatus\\t%s\\n\", c, c/100, st; if(sz!=\"\") printf \"size\\t%s\\n\", sz}'"]
          ...
          Row {
            ...
            Repeater {
              ...
              Button {
                ...
                text: modelData === "power-saver" ? "Battery Saver"
                  : (modelData === "balanced" ? "Balanced" : "Performance")
