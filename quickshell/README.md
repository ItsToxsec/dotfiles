...
## NixOS fixes for Quickshell 0.3.0

The NixOS port does not require `uwsm-app`, `gtk-launch`, or Omarchy's original CLI. Application launching uses Quickshell 0.3.0 `DesktopEntry.execute()`. Battery/power helpers are invoked by absolute paths inside this shell directory, using `/sys/class/power_supply/BAT1` for the battery. The `bin/omarchy-shell` compatibility wrapper translates Omarchy shell commands to Quickshell 0.3.0 `qs ipc call` commands.

Test the shell IPC with:

```bash
qs ipc show
qs ipc call shell ping
qs ipc call shell toggle omarchy.menu '{"menu":"root"}'
```

If your executable is named `quickshell` instead of `qs`, the bundled `bin/omarchy-shell` wrapper detects and uses it automatically.
